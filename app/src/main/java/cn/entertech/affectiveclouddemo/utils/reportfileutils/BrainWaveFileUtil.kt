...
class BrainWaveFileUtil ()  {
    ...
    fun writeReportFile(fileName: String, byteArray: ByteArray,sleepType: String){
        val meditationReport = File(getReportDir())
     ...
     * 写文件(新文件协议);
     */
    fun <T: BrainDataUnit> writeFragment(fileName: String, fragment: FileFragment<T>) {
        ...
        when {
            brainUnit is MeditationReportDataAnalyzed -> {
                writeReportFile(fileName, fragment.getWriteData(),"meditation")
            }
            else -> {}
        }
    }
    ...
    fun getReportDir(): String {
        return FileUtil.getFilesDir().toString() + "/" + Constants.MEDITATION_REPORT_FILE
    }
