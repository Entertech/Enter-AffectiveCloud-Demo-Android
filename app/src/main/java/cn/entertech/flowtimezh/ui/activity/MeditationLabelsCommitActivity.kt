...
class MeditationLabelsCommitActivity : BaseActivity() {
    private var meditationLabels: MutableList<MeditationLabelsModel>? = null
    ...
    fun initView() {
        ...
        meditationLabels = meditationLabelsDao.findByMeditationId(meditationId)
        ...
        adapter!!.onItemChildClickListener =
            BaseQuickAdapter.OnItemChildClickListener { adapter, view, position ->
                ...
                intent.putExtra(Constant.EXTRA_LABEL_ID, meditationLabels!![position].id)
        ...
        btn_commit.setOnClickListener {
            if (isAllLabelRecord()) {
                ...
                Toast.makeText(this,"还有未选择标签的记录!",Toast.LENGTH_SHORT).show()
            }
        }
    }

    fun isAllLabelRecord(): Boolean {
        var result = true
        if (!meditationLabels.isNullOrEmpty()) {
            for (label in meditationLabels!!) {
                if (label.dimIds == null) {
                    result = false
                }
            }
        }
