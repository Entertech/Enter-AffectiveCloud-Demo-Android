...
package cn.entertech.flowtimezh.ui.view.scrolllayout;
...
public class ScrollLayout extends FrameLayout {
    ...
    private static final float DRAG_SPEED_MULTIPLIER = 1.4f;
...
//                        Log.d("open", "2");
    ...
    private boolean isChildListView = false;
    private float lastX;
    private float lastY;
    ...
    private boolean isChildListTop = true;
    ...
    private void initFromAttributes(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.ScrollLayout_isChildListView)) {
            isChildListView = a.getBoolean(R.styleable.ScrollLayout_isChildListView, false);
        }
...
//            Log.d("ScrollLayout", -y + " <=" + maxOffset);
        } else {
...
//            Log.d("computeScroll", "currY:" + currY);
    ...
    public void setIsChildListTop(boolean flag) {
        isChildListTop = flag;
    }

    public void setTouchOffset(int touchOffset) {
        mTouchOffset = touchOffset;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        /**判断触碰点的位置,在标题的上方是负数,
         * getMeasuredHeight()-exitOffset是标题的高度
         * 点击范围在标题外就不触发事件
         */
        if ((y < mTouchOffset) && (y > 0)) {
            isCurrentPointerIntercepted = true;
//            Log.d("ScrollLayout", "Y:" + ev.getY() + "  RY:" + ev.getRawY() + " exitOffset:" + (getScreenHeight() - exitOffset)
//                    + "  scrollY:" + getScrollY() + "  yy:" + (ev.getY() + getScrollY()));
            return true;
        }
        if (isChildListView) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                return true;
            case MotionEvent.ACTION_MOVE:
//                Log.d("move", "lastFlingStatus: " + lastFlingStatus);
...
//                    Log.d("move", "3");
            ...
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL:
//                Log.d("ScrollLayout", "currentInnerStatus" + currentInnerStatus);
                if (currentInnerStatus == InnerStatus.MOVING) {
                    completeMove();
                    return true;
                } else if (currentInnerStatus == InnerStatus.OPENED) {
                    isScrollToClose = true;
                    scrollToClose();//自定义操作：点击展开
                } else if (currentInnerStatus == InnerStatus.CLOSED || currentInnerStatus == InnerStatus.EXIT) {
                    scrollToOpen();//自定义操作：点击关闭
                }
                break;
            default:
                return false;
    ...
    private boolean isScrollToClose = false;
...
//        Log.d("move", getId() + "to open" + getVisibility());
        if (currentInnerStatus == InnerStatus.OPENED) {
            return;
        }
        if (maxOffset == minOffset) {
