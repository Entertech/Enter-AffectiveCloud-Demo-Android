...
//        Logger.d("fileProtocol size is " + fileProtocol.list.size)
        if (fileProtocol == null || fileProtocol.getList() == null || fileProtocol.getList().size() <= 0) {
            if (fileProtocol == null) {
                Logger.d("error record id : " + userLessonEntity.getId());
            }
            return null;
        }
    ...
    public boolean isFileExist(UserLessonEntity userLessonEntity){
        if (userLessonEntity.getMeditation() == 0L) {
            return false;
        }
        MeditationDao meditationDao = new MeditationDao(Application.Companion.getInstance());
...
//        Logger.d("file name is " + fileName)
    ...
    public UserLessonEntity findRecordByMeditationId(int userId, long meditationId) {
        try {
            ...
            List<UserLessonEntity> records = mRecordDaoOp.queryBuilder().where().eq("meditation_id", meditationId).and().eq("user", userId).and().eq("is_delete", false).query();
            if (!records.isEmpty()) {
